...
do
    case $arg in
        ...
        cd ${DATE}
        if [ ! -f "${DATE}.json" ]; then 
            wget "https://patchwork.kernel.org/api/series/?project=365&archive=both&format=json&before=${DATE}T23%3A59%3A59&since=${DATE}T00%3A00%3A00" -O ${DATE}.json
        fi
        ...
        do
            ...
            # cover_letter
            wget "https://patchwork.kernel.org/api/covers/?project=365&format=json&archive=both&before=${DATE}T23%3A59%3A59&since=${DATE}T00%3A00%3A00" -O covers.json
